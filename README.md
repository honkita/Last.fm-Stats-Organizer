[![](https://raw.githubusercontent.com/honkita/PixelButtons/main/Pixel_GitHub.svg)](https://github.com/honkita) [![](https://raw.githubusercontent.com/honkita/PixelButtons/main/Pixel_Link.svg)](https://elitelu.com) [![](https://raw.githubusercontent.com/honkita/PixelButtons/main/Pixel_LinkedIn.svg)](https://www.linkedin.com/in/elitelu/)

# Last.<!-- -->fm Stats Organizer

![](https://raw.githubusercontent.com/honkita/PixelButtons/main/Pixel_Maintained.svg) ![](https://raw.githubusercontent.com/honkita/PixelIcons/main/Languages/TypeScript.svg) ![](https://raw.githubusercontent.com/honkita/PixelIcons/main/Languages/CSS.svg) ![](https://raw.githubusercontent.com/honkita/PixelIcons/main/Frameworks/Nextjs.svg)

---

## About

Originally, this project was a simple script to fetch and clean my Last.<!-- -->fm data for personal use. However, I decided to turn it into a web application to make it accessible to others who might want to do the same with their Last.<!-- -->fm data. The app allows users to input their Last.<!-- -->fm username and API key, fetches their listening history, and organizes it in a more user-friendly way.

## Versioning

I use semantic versioning for this project. The version number is in the format of `MAJOR.MINOR.PATCH`.

### MAJOR

- New big features such as adding the artist search.

### MINOR

- Smaller enhancements and bug fixes.
- Regex repairs

### PATCH

- Small bug fixes.

## FAQ

### Why?

Last.<!-- -->fm has been very bad with their API and the way they store music. I haven't even TOUCHED the actual song data, and I am extremely worried about doing that since that would be a literal nightmare. However, the artist data is already a mess, and I want to have it organized in a way that makes sense to me. This project is a way for me to take control of my own data and make it usable for my own purposes.
